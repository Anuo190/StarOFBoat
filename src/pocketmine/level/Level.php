    ...
    */
    ...
    public function updateBlockLight($x, $y, $z)
    {
        ...
        while (!$lightRemovalQueue->isEmpty()) {
            ...
            $this->computeBlockLight($node, $lightLevel, $lightRemovalQueue, $lightPropagationQueue, $removalVisited, $visited);
        }
    ...
    private function computeBlockLight($node, $lightLevel, $lightRemovalQueue, $lightPropagationQueue, $removalVisited, $visited)
    {
    ...
    {
        if ($this->getServer()->weatherEnabled == True && $this->getDimension() == self::DIMENSION_NORMAL) {
            if ($this->getWeather() === self::WEATHER_CLEARSKY) {
                $random = mt_rand(1, 1000000);
                if ($random <= $this->rainprob) {
                    Server::getInstance()->getPluginManager()->callEvent($ev = new WeatherChangeEvent($this, self::WEATHER_CLEARSKY,1));
                    if (!$ev->isCancelled()) {
