					...
					if ($this->AIHolder->willMove($zo)) {
						...
						if ($zom['IsChasing'] === false) {  //自由行走模式
							if ($zom['gotimer'] == 0 or $zom['gotimer'] == 10) {
								...
								while (abs($newmx - $zom['motionx']) >= mt_rand(-5, 5) / 10) {
									$newmx = mt_rand(-5, 5) / 10;
								}
								$zom['motionx'] = $newmx;

								$newmz = mt_rand(-5, 5) / 10;
								while (abs($newmz - $zom['motionz']) >= mt_rand(-5, 5) / 10) {
									$newmz = mt_rand(-5, 5) / 10;
								}
								$zom['motionz'] = $newmz;

								if ($zom['gotimer'] >= 20 and $zom['gotimer'] <= 22) {
			...
			foreach ($level->getEntities() as $zo) {
				if ($zo instanceof Zombie) {
					...
					if (0 < $level->getTime() and $level->getTime() < 13500) {
						...
						if ($this->AIHolder->whatBlock($level, new Vector3($zo->getX(), floor($zo->getY() - 1), $zo->getZ())) == "water")
							$ok = false;
						if ($ok)
							$zo->setOnFire(2);
					}
