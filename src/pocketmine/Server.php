...
class Server
{
	...
	public $WorldProtect = "";
	public $ReleaseMemLimit = 1000000;
	public $specialgenerator = true;
	public $critical = true;
	public $trafficThreshold = 1000000;
	...
	public $antiautogap = true;
	public $antiautogaptime = 5;
	public $antireach = true;
	public $antireachdistance = 4.5;
	public $antiautoclicker = true;
	public $antiautoclickercountdown = 20;
	public $antiflyprecision = 10;
	public $antifly2 = true;
	...
	{
		return max(56, $this->getProperty("chunk-sending.max-chunks", 489));
	}
	...
	public function getOfflinePlayerData($name)
	{
		...
		$defaultLevel = $this->getDefaultLevel();
		...
		if (file_exists($path . "$name.dat")) {
			try {
				...
				$logger->notice($language->translateString("pocketmine.data.playerCorrupted", [$name]));
			}
		} else {
			$logger->notice($language->translateString("pocketmine.data.playerNotFound", [$name]));
		}
		$spawn = $defaultLevel->getSafeSpawn();
		$nbt = new CompoundTag("", [
			...
			new StringTag("Level", $defaultLevel->getName()),
	...
	{
		try {
			$this->nbt->setData($nbtTag);

			if ($async) {
				$this->getScheduler()->scheduleAsyncTask(new FileWriteTask($this->getDataPath() . "players/" . strtolower($name) . ".dat", $this->nbt->writeCompressed()));
			} else {
				file_put_contents($this->getDataPath() . "players/" . strtolower($name) . ".dat", $this->nbt->writeCompressed());
			}
		} catch (\Throwable $e) {
	...
	public function loadAdvancedConfig()
	{
		...
		$this->WorldProtect = $this->getAdvancedProperty("server.Wprotect", "");
		...
		$this->antireach = $this->getAdvancedProperty("server.anti-reach", true);
		$this->antiautogap = $this->getAdvancedProperty("server.anti-autogap", true);
		$this->antifly2 = $this->getAdvancedProperty("server.antiflymode2", true);
		$this->antiautoclicker = $this->getAdvancedProperty("server.anti-ac", true);
		$this->specialgenerator = $this->getAdvancedProperty("server.specialgenerator", true);
		$this->antiautoclickercountdown = $this->getAdvancedProperty("server.around", 20);
		$this->trafficThreshold = $this->getAdvancedProperty("server.traffic-Threshold", 1000000);
		$this->critical = $this->getAdvancedProperty("server.critical", true);
		$this->antiautogaptime = $this->getAdvancedProperty("server.antigap-time", 5);
		$this->antiflyprecision = $this->getAdvancedProperty("server.precision", 10);
		$this->ReleaseMemLimit = $this->getAdvancedProperty("server.MemoryLimit2", 1000000);
		$this->antireachdistance = $this->getAdvancedProperty("server.distance", 4.5);
		...
		try {
			...
			$this->aboutstring = '
				...
				|  $$$$$$ |_  $$_/   |____  $$ /$$__  $$| $$  | $$| $$$$    | $$$$$$$  /$$__  $$ |____  $$|_  $$_/  
				 ...
				 /$$  \ $$  | $$ /$$ /$$__  $$| $$      | $$  | $$| $$      | $$  \ $$| $$  | $$ /$$__  $$  | $$ /$$
				|  $$$$$$/  |  $$$$/|  $$$$$$$| $$      |  $$$$$$/| $$      | $$$$$$$/|  $$$$$$/|  $$$$$$$  |  $$$$/
				 \______/    \___/   \_______/|__/       \______/ |__/      |_______/  \______/  \_______/   \___/                       MMMMMMMMMMM        M#########M
			';
	...
	private function titleTick()
	{
		...
		echo "\x1b]0;" . $this->getName() . $this->getFormattedVersion("-") .
			...
			" | 内存 " . $usage .
			" | 上行 " . round($this->network->getUpload() / 1024, 2) .
			" 下行 " . round($this->network->getDownload() / 1024, 2) .
			" kB/s | TPS " . $this->getTicksPerSecondAverage() .
			" | 负载 " . $this->getTickUsageAverage() . "%\x07";
