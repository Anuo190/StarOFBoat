...
class Chest extends Spawnable implements InventoryHolder, Container, Nameable{
    ...
    public function close(){
        if($this->closed === false){
            ...
            $realInventory = $this->getRealInventory();
            foreach($viewers as $player){
    ...
    public function saveNBT(){
        ...
        for($index = 0; $index < $this->getSize(); ++$index){
            ...
            if($item->getId() !== Item::AIR and $item->getCount() > 0){
                $items[] = NBT::putItemHelper($item, $index);
            }
        }
     ...
     * @return int
     */
    public function getSlotIndex($index): int
    {
        if(isset($this->slotIndexMap[$index])){
            return $this->slotIndexMap[$index];
        }
        foreach($this->namedtag->Items as $i => $slot){
            if((int) $slot["Slot"] === (int) $index){
                $this->slotIndexMap[$index] = (int) $i;
                return (int) $i;
            }
        }
        return -1;
